...
class HashJoinExecutor : public Executor {
  ...
  private:
    ...
    const catalog::Schema *left_schema_, *right_schema_;
    Executor *left_child_executor_, *right_child_executor_;
    // std::unordered_map<type::Value, storage::Tuple>::iterator hash_iter_;
};
}  // namespace naivedb::query